...
@Component({
  selector: 'app-identity-mode',
  templateUrl: './identity-mode.component.html',
  styleUrls: ['./identity-mode.component.sass'],
})
export class IdentityModeComponent {
  tableData: any = [];
  pageSize: any = 5;
  pageNumber: any = 1;
  totalRecords: any;
  ...
  getAllIdentityMode() {
    ...
    this.subAgentSetting
      .getSubAgentagentIdentityMode(data)
  ...
  changeRoleStatus(values: any) {
    ...
    data['agentIdentityModeId'] = values.agentIdentityModeId;

    this.subAgentSetting.updateSubAgentIdentityModeStatus(data).subscribe({
      next: (res) => {
