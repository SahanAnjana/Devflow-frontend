...
export class AddNotesComponent {
  ...
  totalRecords: any;
  currentUser: any;
  transactionDetailsIdForAddNote: any;
  emailForAddnote: any;
  Object: any;
  constructor(
  ...
  ngOnInit() {
    ...
    // console.log('agentTransactionMasterId', this.mode);
  }
  getSubject() {
  ...
  getNotes() {
    const data: any = {};
    data['transactionDetailId'] = this.mode.agentTransactionDetailId;
    ...
    this.reportNotes.getAllNotes(data).subscribe({
      next: (res) => {
        if (res['responseDto']) {
          this.Object = res['responseDto'] ? res['responseDto'] : '';
          this.notesData = res['responseDto']['payload']
            ? res['responseDto']['payload']
            : '';
  ...
  addNote() {
    ...
    const data = {
      ...
      agentTransactionDetailId: this.mode.agentTransactionDetailId,
      createdBy: this.mode.userEmailAddress,
    };
  ...
  //   const sanitizedText = this.sanitizer.sanitize(0, text) || ''; // Sanitize HTML or return empty string if null
  //   const div = document.createElement('div');
  //   div.innerHTML = sanitizedText;
  //   const plainText = div.textContent || div.innerText || ''; // Get plain text
  //   return plainText;
  // }
